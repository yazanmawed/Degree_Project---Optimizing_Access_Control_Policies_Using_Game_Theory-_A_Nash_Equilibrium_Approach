...
def run_game_theory_analysis():
    ...
    defender_payoffs = [[3, 1],
                        [-1, 2]]

    attacker_payoffs = [[1, 2],
                        [3, 1]]
...
class EmployeeAgent(Agent):
    def __init__(self, unique_id, model):
        ...
        if np.random.rand() < 0.3:
            self.role = np.random.choice(["authorized_role_1", "authorized_role_2"])
        else:
    ...
    def attempt_access(self):
        rbac_weight, abac_weight = self.model.policy_mix
        if np.random.rand() < rbac_weight:
            return self.rbac_access()
        else:
            return self.abac_access()

    def rbac_access(self):
        return self.role in ["authorized_role_1", "authorized_role_2"]
...
class AttackerAgent(Agent):
    ...
    def execute_attack(self):
        ...
        if self.attack_strategy == "phishing":
            base_success = 0.3 * rbac_weight + 0.5 * (1 - rbac_weight)
...
class DefenderAgent(Agent):
    def __init__(self, unique_id, model):
        ...
        # معاملات التحكم في تعديل السياسة - تم تقليلها لجعل التغيير أكثر تدريجية
        self.K_s = 0.15  # معامل الأمان (تم تقليله من 0.3)
        self.K_u = 0.05  # معامل سهولة الاستخدام (تم تقليله من 1.0)

        # معامل التخميد لجعل التغييرات أكثر سلاسة
        self.damping_factor = 0.7  # قيمة بين 0 و 1، حيث 1 تعني لا تخميد و 0 تعني لا تغيير
